...
The etaHEN plugin (title **NTPR50001**, version **0.571**) owns freeze and cheats when armed so they keep applying with the web UI closed. From **0.571** it injects `overlay.elf` into the launching game via an int3 stager (not `pt_call` of `scePthreadCreate`). This Windows PC does **not** compile it. GitHub Actions does (Ubuntu + Johns SDK).
