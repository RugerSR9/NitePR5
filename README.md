...
Phases 0–4 are **done**. Phase 5 overlay is **code-complete** (not hardware-verified): plugin **NTPR50001** 0.56 auto-injects `overlay.elf` at game launch via Johns elfldr. The web UI is the shippable PC editor; the plugin owns freeze on-console (a web poke is overwritten). After a hung scan or write (`timed out reading 4 bytes`), **Disconnect then Connect** (or restart uvicorn). The rest-mode hint is often a false alarm from a desynced `:744` socket.
...
The etaHEN plugin (title **NTPR50001**, version **0.56**) owns freeze and cheats when armed so they keep applying with the web UI closed. From **0.56** it injects `overlay.elf` into the launching game via Johns elfldr (FPS path: attach, map, push RIP, detach). This Windows PC does **not** compile it. GitHub Actions does (Ubuntu + Johns SDK).
