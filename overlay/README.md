...
1. CI: download `nitepr5.plugin` (**0.53**) and `overlay.elf`.
...
6. Start **CUSA13762** (or another CUSA/PPSA title). Wait ~12 s for the toast **overlay injected**, then **overlay entry is up** (plugin heartbeat). If you get **overlay silent (never started)**, the ELF was mapped but the overlay thread never started — copy the matching CI `overlay.elf`. The game must keep booting (0.52 parked the stolen thread and froze launch).
...
The plugin waits ~12 s after a game title appears (8 s if a game is already running when NTPR50001 starts), finds `eboot.bin` via `:744`, then `PROC_ELF`. Retry up to 5 times if the eboot is not up yet.
