...
1. CI: download `nitepr5.plugin` (**0.56**) and `overlay.elf`.
...
The plugin waits ~12 s after a game title appears (8 s if a game is already running when NTPR50001 starts), finds `eboot.bin` via `:744`, disconnects that socket, then Johns `elfldr_exec` (attach, map, push RIP, detach). Retry up to 5 times if the eboot is not up yet.
