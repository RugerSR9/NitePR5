...
| Phase 5 | **B3** `code_complete`. Plugin 0.52 auto-inject + overlay 0.52 custom `overlay_start` e_entry. Hardware: inject toast seen; overlay toasts missing on 0.51. |
...
Phase 4 is **`done`**. Phase 5 backend is **B3**, **code_complete** (not hardware-done). Plugin **0.52** auto-injects `/data/nitepr5/overlay.elf` into `eboot.bin` via ps5debug-NG `PROC_ELF` (~4 s after CUSA/PPSA launch). Overlay **0.52** uses custom `overlay_start` (PROC_ELF does not wait for `main()`; Johns CRT often never reaches it in a live eboot). CI: `nitepr5.plugin` and `overlay.elf`. Do not send either to elfldr **9021**.

### Phase 4 plugin tree

`plugin/` exists. Title NTPR50001 / **0.52**. HTTP :1745. Two-phase PROC_WRITE + one-phase PROC_READ + two-phase PROC_ELF to 127.0.0.1:744. After inject, plugin stats `/data/nitepr5/overlay.alive` (~2 s) and toasts entry-up vs silent. ELF is built in GitHub Actions (`bash plugin/build.sh`), not on this Windows PC.
...
| Phase | Job |
|---|---|
| 5 | **B3** `code_complete`. Hardware: plugin auto-inject overlay.elf into CUSA13762; poke one value. |
