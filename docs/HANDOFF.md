...
| Phase 5 | **B3**. 0.571 inject+combo passed. 0.572 CE at inject. 0.573 launches; pointer-match GOT is a no-op. **0.575** + plugin wrap **0.57**. |
| Version | Overlay **0.575**. Plugin wrap **0.57** until HUD works. Do not bump wrap. |
...
- etaHEN `.plugin` header: historically `^\d\.\d{2}$`. `plugin/CMakeLists.txt` **`PLUGIN_VERSION 0.57`**. Do not bump wrap until HUD works.
- `make_plugin.py` also allows `x.xxx`. Prefer **`x.xx` on the plugin wrap** so Toolbox loads.
- `overlay/include/overlay.h` **`OVERLAY_VERSION 0.575`**. `plugin/include/nitepr5.h` **`0.575`** (string); wrap uses CMake **0.57**.

### Files that matter

- Plugin inject: `plugin/source/inject.c`, `plugin/third_party/elfldr/{elfldr.c,pt.c}` (`elfldr_inject`)
- Overlay entry: `overlay/source/start.c` (`overlay_gate`), `overlay/source/main.c` (hooks **off** at launch, never returns)
- HUD: `overlay/source/{hooks,draw,hud,worker}.c` — pad poll only; plugin `got_patch.c` NID-PLT on `/overlay/open`
...
See **Next orchestrator — start here** at the top. Phase 4 is **`done`**. Next is **hardware** for overlay **0.575** + plugin wrap **0.57**. Do not send overlay to elfldr **9021**. Do not NineS `:9033`.
...
| **B3** in-game ELF | **GO.** 0.571 inject+combo **passed**. 0.572 **CE at inject**. 0.573 **launches**; pointer-match GOT no-op. **0.575** overlay hooks off; plugin wrap **0.57** NID-PLT on combo. |
...
5. `docs/ORCHESTRATION.md` Phase 5 — next is **hardware** for 0.575 overlay + 0.57 plugin wrap; do not hook GNM from overlay.elf
...
| Phase | Job |
|---|---|
| 5 | **B3** 0.571 inject+combo **passed**. 0.572 CE at inject. 0.573 launches; pointer-match GOT no-op. **0.575** + plugin wrap **0.57** NID-PLT on combo. Overlay spike is not optional. |
