...
| 2026-08-27 | Overlay **0.572** hardware: **CE-108255-1** during inject. Inline `detour_install` `mprotect`+jmp on GNM/VO SPRX text after `elfldr_inject` restored game caps. **0.573**: no hooks in `main()`; eboot GOT/JMPREL pointer swap after pad poll; `orig` is the SPRX export (no trampoline). Do not revive SPRX body hooks. Do not raise authid. |
| 2026-08-27 | Overlay **0.573** still **CE-108255-1 at launch**. Pad poll succeeds during boot, so in-game `kernel_dynlib_*` / GOT write is still in the inject window. **0.574**: overlay never installs hooks (0.571). Plugin **0.58** patches eboot GOT on combo only (`kernel_proc_copyout`/`setlong` from the jailbroken daemon). |
