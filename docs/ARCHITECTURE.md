...
| 2026-08-27 | 0.571 hardware: running → pad poll ok → injected → CE-108255-1. `pt_call` must not be used for `scePthreadCreate` (steps the new thread, restores game regs onto it). Next drop (still **0.571**): int3 stager + overlay_gate delay; GNM hooks off. |
| 2026-08-27 | Overlay **0.572**: install VideoOut + GNM flip hooks; keep DualSense on pad poll (`overlay_on_input` when HUD open). Do **not** detour `scePadReadState`. Plugin wrap stays **0.57**. Hardware panel not run. |
| 2026-08-27 | Overlay **0.572** hardware: **CE-108255-1** during inject. Inline `detour_install` `mprotect`+jmp on GNM/VO SPRX text after `elfldr_inject` restored game caps. **0.573**: no hooks in `main()`; eboot GOT/JMPREL pointer swap after pad poll; `orig` is the SPRX export (no trampoline). Do not revive overlay SPRX body hooks. Do not raise authid. |
| 2026-08-27 | Overlay **0.573** was blamed for launch CE; user still had **0.572** files. **0.573 launches.** Combo on plugin **0.58** toasts **`got flip=0 vo=0`**: matching eboot GOT by `*slot == dlsym(real)` is a no-op (lazy PLT / eboot-only). **0.575**: overlay still never installs hooks. Plugin wrap **0.57** NID-PLT (`imagebase+r_offset`, all dynlib handles) on combo. Plugin combo SPRX trampoline (`s=1`) **CE-108255-1** after toggles; reboot. Do not revive SPRX. Do not bump wrap until HUD works. |
