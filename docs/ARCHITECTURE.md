...
| 2026-08-27 | Overlay **0.573** was blamed for launch CE; user still had **0.572** files. **0.573 launches.** Combo on plugin **0.58** toasts **`got flip=0 vo=0`**: matching eboot GOT by `*slot == dlsym(real)` is a no-op (lazy PLT / eboot-only). **0.575**: overlay still never installs hooks. Plugin wrap **0.57** NID-PLT (`imagebase+r_offset`, all dynlib handles) on combo; privileged SPRX trampoline only if PLT writes 0. Do not bump wrap until HUD works. |
